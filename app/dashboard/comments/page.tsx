...
const Comments = () => {
  ...
  return (
    ...
    {commentsAll?.map((comment, index) => (
      <div
        key={comment._id}
        className='w-full flex flex-wrap items-center justify-start gap-4  text-xl border-b-2'
      >
        <p>{index + 1}.</p>
        <p>Id: {comment.postId}</p>
        <p className='underline capitalize'>{comment.name}</p>
        <p>{comment.content}</p>
        <ButtonDelete id={comment._id.toString() } deleteElementId={deleteCommentId} />
      </div>
    ))}
  </div>
  )
